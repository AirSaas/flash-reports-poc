...
const MAX_POLL_TIME_GENERATION = 5 * 60 * 1000 // 5 minutes max for generation
...
export function useGenerate(sessionId: string, engine: Engine | null): UseGenerateReturn {
  ...
  /**
   * Poll for Claude PPTX generation job completion (Python backend)
   */
  const pollPptxJobStatus = useCallback(
    async (jobId: string, startTime: number): Promise<GenerationResult | null> => {
      ...
      switch (job.status) {
        ...
        case 'processing':
          ...
          return pollPptxJobStatus(jobId, startTime)
  ...
  /**
   * Poll for Gamma generation job completion (Supabase)
   */
  const pollGammaJobStatus = useCallback(
    async (jobId: string, startTime: number): Promise<GenerationResult | null> => {
      if (Date.now() - startTime > MAX_POLL_TIME_GENERATION) {
        throw new Error('Generation timed out. Please try again.')
      }
      ...
      switch (job.status) {
        ...
        case 'processing':
          ...
          return pollGammaJobStatus(jobId, startTime)
  ...
  const generateGamma = useCallback(async (): Promise<GenerationResult | null> => {
    ...
    try {
      ...
      const startTime = Date.now()
      const generationResult = await pollGammaJobStatus(response.jobId, startTime)
  ...
  }, [engine, generatePptx, generateGamma])
